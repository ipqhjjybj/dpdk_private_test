...
static void process_depth_data(const unsigned char *data, int len) {
    ...
    char *lastUpdateId = strstr(json_str, "\"lastUpdateId\":");
    char *updateId = strstr(json_str, "\"u\":");
    char *firstUpdateId = strstr(json_str, "\"U\":");
...
{
    printf("F-Stack + mbedTLS 币安WebSocket客户端 (深度数据 + 微秒级延迟测试)\n");
    printf("=======================================================\n");
    printf("功能: 实时接收BTC/USDT订单簿深度数据并测试网络延迟\n");
