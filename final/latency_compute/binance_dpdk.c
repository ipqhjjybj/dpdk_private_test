...
typedef struct {
    ...
    double max_latency;
    int count;
    double total_rx_latency;  // RX层延迟（网卡到应用）
    double min_rx_latency;
    double max_rx_latency;
} latency_stats_t;

latency_stats_t latency_stats = {0.0, 999999.0, 0.0, 0, 0.0, 999999.0, 0.0};

// 包接收时间戳（记录数据从网卡到达的时间）
static double packet_rx_timestamp = 0.0;
...
// 更新延迟统计
static void update_latency_stats(double latency_us, double rx_latency_us) {
...
static int fstack_net_recv(void *ctx, unsigned char *buf, size_t len) {
    ...
    if (ret > 0) {
        ...
        if (ff_get_last_rx_timestamp(&rx_timestamp_ns) == 0 && rx_timestamp_ns > 0) {
            // 转换为微秒并保存为全局变量
            packet_rx_timestamp = (double)rx_timestamp_ns / 1000.0;
        }
    }
...
// 旧版本的WebSocket帧解析（保留以防兼容性问题）
static int parse_websocket_frame_legacy(const unsigned char *buffer, int len,
...
static void process_trade_data(const unsigned char *data, int len) {
    ...
    // 计算从网卡接收到应用处理的延迟（RX延迟）
    double rx_latency_us = 0.0;
    if (packet_rx_timestamp > 0.0) {
        rx_latency_us = process_time - packet_rx_timestamp;
    }
    ...
    if (latency_stats.count <= 5) {
        printf("DEBUG[%d]: binance_ts=%lld, process_time=%.1f, trade_id=%lld, stream='%s'\n",
               latency_stats.count, binance_timestamp, process_time, trade_id, stream_name);
        printf("DEBUG[%d]: binance_ts_us=%.1f, latency_raw=%.1f, rx_latency=%.1fμs\n",
               latency_stats.count, (double)binance_timestamp * 1000.0,
               process_time - ((double)binance_timestamp * 1000.0), rx_latency_us);
...
int main(int argc, char * argv[])
{
    ...
    printf("================================================================================\n");
    printf("RX延迟测量说明 (完整硬件时间戳支持):\n");
    printf("  测量起点: 数据包到达网卡 (DPDK硬件时间戳)\n");
    printf("  测量终点: 应用层处理完成\n");
    printf("  包含延迟: 网卡DMA + DPDK处理 + F-Stack网络栈 + ff_recv() + \n");
    printf("            SSL解密 + WebSocket解帧 + 应用处理\n");
    printf("  时间戳来源: rte_mbuf->m_pkthdr.rcv_tstmp (纳秒精度)\n");
    printf("  注意:     这是从网卡到应用的完整端到端延迟测量\n");
    printf("================================================================================\n\n");
