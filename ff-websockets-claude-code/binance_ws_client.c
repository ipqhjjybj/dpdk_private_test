...
static int fstack_net_send(void *ctx, const unsigned char *buf, size_t len) {
    ...
    if (ret < 0) {
        if (ff_errno == EAGAIN || ff_errno == EWOULDBLOCK) {
...
static int fstack_loop(void *arg) {
    ...
    // 简化DNS解析，直接使用IP（实际应用中需要DNS解析）
    if (inet_pton(AF_INET, "52.84.25.84", &server_addr.sin_addr) <= 0) {
        printf("IP地址转换失败\n");
        goto cleanup;
    }
    
    printf("正在连接到 %s:%d...\n", BINANCE_HOST, BINANCE_PORT);
    if (ff_connect(sockfd, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
        printf("连接失败: %d\n", ff_errno);
        goto cleanup;
    }
...
int main(int argc, char *argv[]) {
    ...
    // 运行主循环
    int result = ff_run(fstack_loop, NULL);
