 ...
 */
 ...
 typedef struct {
     ...
     // WebSocket缓冲区
     unsigned char ws_buffer[131072];
     int ws_buffer_len;
 ...
 static void process_tcp_packet(struct rte_mbuf *m) {
     ...
     // 状态机处理
     printf("DEBUG: 当前TCP状态=%d, 收到flags=0x%02x, seq=%u, ack=%u, data_len=%u\n",
            conn_ctx->tcp_state, flags, seq, ack, tcp_data_len);
         ...
         case TCP_ESTABLISHED:
             if (tcp_data_len > 0) {
                 ...
                 if (conn_ctx->rx_buffer_len + tcp_data_len < sizeof(conn_ctx->rx_buffer)) {
                     ...
                     // TLS解密处理
                     printf("DEBUG: TLS状态=%d, rx_buffer_len=%d\n", conn_ctx->tls_state, conn_ctx->rx_buffer_len);
                     if (conn_ctx->tls_state == TLS_HANDSHAKING) {
                         int ret = mbedtls_ssl_handshake(&conn_ctx->ssl);
                         printf("DEBUG: mbedtls_ssl_handshake返回值=%d\n", ret);
                         if (ret == 0) {
                             printf("✓ TLS握手完成\n");
                             ...
                             const char *ws_handshake =
                                 "GET /stream?streams=btcusdt@trade/ethusdt@trade HTTP/1.1\r\n"
                             ...
                             printf("DEBUG: TLS需要更多数据 (WANT_READ)\n");
                         } else if (ret == MBEDTLS_ERR_SSL_WANT_WRITE) {
                             printf("DEBUG: TLS需要发送数据 (WANT_WRITE)\n");
                         } else {
                             char error_buf[100];
                         ...
                         printf("DEBUG: mbedtls_ssl_read返回值=%d\n", ret);
                         if (ret > 0) {
                             printf("✓ 收到解密数据: %d字节\n", ret);
 
                             // 打印前200字节的内容（用于调试）
                             printf("--- 解密数据内容 (前%d字节) ---\n", ret > 200 ? 200 : ret);
                             for (int i = 0; i < (ret > 200 ? 200 : ret); i++) {
                                 if (decrypted[i] >= 32 && decrypted[i] <= 126) {
                                     printf("%c", decrypted[i]);
                                 } else if (decrypted[i] == '\r') {
                                     printf("\\r");
                                 } else if (decrypted[i] == '\n') {
                                     printf("\\n\n");
                                 } else {
                             ...
                             if (conn_ctx->ws_buffer_len + ret < sizeof(conn_ctx->ws_buffer)) {
                                 ...
                                 if (frame_size > 0) {
                                     printf("✓ WebSocket帧: opcode=%d, payload_len=%d\n", opcode, payload_len);
                                     if (opcode == WS_OPCODE_TEXT) {
                                         process_trade_data(payload, payload_len, hw_ts);
                                     } else if (opcode == WS_OPCODE_PING) {
                                         printf("收到WebSocket PING\n");
 ...
 static int send_websocket_frame(int opcode, const unsigned char *payload, int payload_len) {
     ...
     return mbedtls_ssl_write(&conn_ctx->ssl, frame, frame_len);
 }
